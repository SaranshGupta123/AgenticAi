...
type MindmapData = {
  ...
  layout_suggestions: { recommended_layout: string; [key: string]: any };
};
...
function buildTree(data: MindmapData): MindmapNode | null {
  ...
  const nodeMap = new Map<string, MindmapNode>(
    nodes.map((n) => [
      n.id,
      { ...n, children: [], isClusterTheme: false } as MindmapNode,
    ])
  );
  ...
  clusters.forEach((c) => {
    ...
    clusterNodes[id] = {
      ...
      color: c.color,
      importance: 9,
      description: `Collection of nodes related to ${c.theme}`,
      keywords: [c.theme.toLowerCase().replace(/\s/g, "_")],
      children: [],
      isClusterTheme: true,
    };
    nodeMap.set(id, clusterNodes[id]);
  });
  ...
  clusters.forEach((c) => {
    const clusterNode = clusterNodes[`cluster_theme_${c.cluster_id}`];
    if (clusterNode) {
      c.node_ids.forEach((nodeId) => {
      ...
      if (!rootNode.children.some((child) => child.id === clusterNode.id))
  ...
  edges
    .filter((e) => e.from !== root_node_id)
    .forEach((e) =>
      (parentChildMap[e.from] = parentChildMap[e.from] || []).push(e.to)
    );

  const resolveChildren = (parentId: string) => {
    const parentObj = nodeMap.get(parentId);
    if (!parentObj) return;
    parentObj.children = (parentChildMap[parentId] || [])
      .map((childId) => nodeMap.get(childId))
      .filter((child): child is MindmapNode => !!child)
      .sort((a, b) => a.label.localeCompare(b.label));
    parentObj.children.forEach((child) => resolveChildren(child.id));
  };
...
export default function MindmapNotebookLM() {
  ...
  const handleMouseMove = (e: React.MouseEvent) =>
    isDragging &&
    setTransform({ x: e.clientX - dragStart.x, y: e.clientY - dragStart.y });
  ...
  const collapseAll = () =>
    setExpandedNodes(new Set(data?.root_node_id ? [data.root_node_id] : []));
  ...
  useEffect(() => {
    ...
    const [levelGap, nodeGap, maxChars] = [450, 90, 40];
    ...
    function drawLinks(node: MindmapNode) {
      ...
      const nodeWidth = Math.min(
        ...
        Math.max(
          200,
          (node.label.length > maxChars ? maxChars : node.label.length) * 9 + 40
        )
      );

      node.children.forEach((child) => {
        const [sourceX, sourceY] = [(node.x || 0) + nodeWidth, node.y || 0];
        const [targetX, targetY] = [(child.x || 0) - 18, child.y || 0];
